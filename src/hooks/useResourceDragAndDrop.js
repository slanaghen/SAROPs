...
export const useResourceDragAndDrop = ({ onDropResource }) => {
  ...
  const handleDrop = async (e, id, type) => {
    ...
    // Always reset state after a drop attempt, regardless of validity
    // This ensures draggedItem and dropTarget are cleared.
    handleDragEnd();

    if (!draggedItem || draggedItem.type === type) {
      return; // Invalid drop, no further action needed
    }
