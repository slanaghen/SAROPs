...
describe('useResourceDragAndDrop Hook', () => {
  ...
  it('handleDrop should not call onDropResource for an invalid drop (same type)', async () => {
    const onDropResource = vi.fn();
    const { result } = renderHook(() => useResourceDragAndDrop({ onDropResource }));
    const mockEvent = createMockEvent();
    ...
    await act(async () => {
      await result.current.handleDrop(mockEvent, 'res-2', 'responder');
    });

    expect(onDropResource).not.toHaveBeenCalled();
    expect(result.current.draggedItem).toBeNull();
    expect(result.current.dropTarget).toBeNull();
  });
});